...
```php
use sant\admin\components\MenuHelper;
...
Return of `sant\admin\components\MenuHelper::getAssignedMenu()` has default format like:
...
return format of `sant\admin\components\MenuHelper::getAssignedMenu()` by provide a callback to this method.
...
Using Sparated Menu
-------------------
Second parameter of `sant\admin\components\MenuHelper::getAssignedMenu()` used to get menu on it's own hierarchy.
...
user sant\admin\components\Helper;
