...
- Enh: Custom side menu via `sant\admin\Module::items` (mdmunir).
...
- Chg: Using `VarDumper::export` to save `data` of `sant\models\AuthItem` (mdmunir).
...
- Chg: Can customize return of `sant\admin\components\AccessHelper::getAssignedMenu()` with provide a callback to method (mdmunir). 
...
- Chg: Remove `sant\admin\components\BizRule` (mdmunir).
...
- Chg: Remove `sant\admin\Module::allowActions`. Set access control directly with `sant\admin\components\AccessControl` (mdmunir).
- Chg: Change cache strategy (mdmunir).
- Chg: `sant\admin\components\DbManager` now inherited from `yii\rbac\DbManager` (mdmunir).
